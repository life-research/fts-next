...
class DeidentifhirUtilTest {
  ...
  void deidentify() throws IOException {
    ...
    Registry registry =
        DeidentifhirUtil.generateRegistry(patient.id(), transportIDs, Duration.ofMillis(1000));
    ...
    Bundle deidentifiedBundle =
        DeidentifhirUtil.deidentify(config, registry, bundle, patient.id(), meterRegistry);
